...
def locate_episode_in_season_torrent(episode, subpath=''):
    ...
    for filename in os.listdir(os.path.join(settings.DOWNLOAD_DIR, torrent_path)):
        filename_fullpath = os.path.join(settings.DOWNLOAD_DIR, torrent_path, filename)
        ...
        elif os.path.isdir(filename_fullpath):
            episode_path = locate_episode_in_season_torrent(episode, os.path.join(subpath, filename))
